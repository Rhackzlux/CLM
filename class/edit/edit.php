<?php
	...
	*/
	class edit
	{
		...
		// public function edit_equipmentstatus($change_status, $change_room, $id, $e_remarks) 
		// {
		// 	global $conn;
		...
		// 			if($checkitemrow > 0) {
		// 				// No update to items_stock based on item_emove needed
		// 				$inveadd = $conn->prepare('INSERT INTO item_inventory (item_id, inventory_status, item_remarks) VALUES (?, ?, ?)');
		// 				$inveadd->execute(array($itemID, $change_status, $e_remarks));
		// 				$invrow = $inveadd->rowCount();
		...
		// 				}
		// 			} else {
		// 				$checkadd = $conn->prepare('INSERT INTO item_stock (item_id, room_id, item_status) VALUES (?, ?, ?)');
		// 				$checkadd->execute(array($itemID, $roomId, $change_status));
		// 				$checkaddrow = $checkadd->rowCount();
		...
		public function return_item($id)
		{
			...
			//dito part yun nahide ko ! kaya nag eerror
...
public function return_item1($id)
{
	global $conn;
	...
	foreach ($fetch as $key => $value) {
		...
		// public function edit_faculty($sid_number, $fname, $lname, $s_gender, $s_contact, $s_type, $app_id)
		// {
		// 	global $conn;
		...
		// 		// Update query
		// 		$updateSql = $conn->prepare('UPDATE member SET m_school_id = ?, m_fname = ?, m_lname = ?, m_gender = ?, m_contact = ?, m_type = ? WHERE id = ?');
		// 		$updateSql->execute(array($sid_number, $fname, $lname, $s_gender, $s_contact, $s_type, $app_id));
		
		// 		// Insert into history_logs
		// 		$insertSql = $conn->prepare('INSERT INTO history_logs(description, table_name, user_id, user_type) VALUES (?, ?, ?, ?)');
		// 		$insertSql->execute(array($h_desc, $h_tbl, $sessionid, $sessiontype));
		
		// 		// Commit the transaction
		// 		$conn->commit();
		
		// 		// Get row count of the last executed query (not reliable for both UPDATE and INSERT in one)
		// 		$row = $updateSql->rowCount();
		// 		echo $row;
		// 	} catch (Exception $e) {
		...
		// 	}
		// }
		public function edit_faculty($sid_number, $fname, $lname, $s_gender, $s_contact, $s_department, $s_type, $app_id)
{
    global $conn;
    ...
    try {
        ...
        // Update query
        $updateSql = $conn->prepare('UPDATE member SET m_school_id = ?, m_fname = ?, m_lname = ?, m_gender = ?, m_contact = ?, m_department = ?, m_type = ? WHERE id = ?');
        $updateSql->execute(array($sid_number, $fname, $lname, $s_gender, $s_contact, $s_department, $s_type, $app_id));
        ...
        // Get row count of the last executed query
        $row = $updateSql->rowCount();
        echo $row;
    } catch (Exception $e) {
        // Rollback transaction on error
...
//faculty
public function activate_faculty($id)
{
	global $conn;
...
//Student
	...
	switch ($key) {
		...
		$app_id = $_POST['app_id'];
		$s_type = $_POST['s_type'];
		$edit->edit_faculty($sid_number,$fname,$lname,$s_gender,$s_contact,$s_department,$s_type,$app_id);
		break;
		...
		case 'activate_faculty';
		$id = $_POST['id'];
		$edit->activate_faculty($id);
		break;

		case 'deactivate_faculty';
		$id = $_POST['id'];
		$edit->deactivate_faculty($id);
		break;
