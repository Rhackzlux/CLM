...
public function display_faculty()
{
	...
	if($count > 0){
		...
		{
			global $conn; 
			$sql = $conn->prepare('	SELECT *, GROUP_CONCAT(item.i_brand, " - " ,item.i_category,  "<br/>") item_borrow FROM borrow
									 LEFT JOIN item_stock ON item_stock.id = borrow.stock_id
									 LEFT JOIN item ON item.id = item_stock.item_id
									 LEFT JOIN member ON member.id = borrow.member_id
									 LEFT JOIN room ON room.id = borrow.room_assigned
		...
		public function display_borrower()
		{
			...
			$sql = "SELECT *, GROUP_CONCAT(item.i_brand, ' - ', item.i_category, '<br/>') item_borrow FROM borrow
					LEFT JOIN item_stock ON item_stock.id = borrow.stock_id
					LEFT JOIN item ON item.id = item_stock.item_id
					LEFT JOIN member ON member.id = borrow.member_id
					LEFT JOIN room ON room.id = borrow.room_assigned";
		...
		{
			global $conn; 
			$sql = $conn->prepare('	SELECT *, GROUP_CONCAT(item.i_brand, " - " ,item.i_category,  "<br/>") item_borrow FROM reservation
								 	LEFT JOIN item_stock ON item_stock.id = reservation.stock_id
								 	LEFT JOIN item ON item.id = item_stock.item_id
								 	LEFT JOIN member ON member.id = reservation.member_id
								 	LEFT JOIN room ON room.id = reservation.assign_room
		...
		public function tbluser_reservation()
		{
			...
			$sql = $conn->prepare('	SELECT *,reservation.status as stat, GROUP_CONCAT(item.i_brand, " - " ,item.i_category,  "<br/>") item_borrow FROM reservation
								 	LEFT JOIN item_stock ON item_stock.id = reservation.stock_id
								 	LEFT JOIN item ON item.id = item_stock.item_id
								 	LEFT JOIN member ON member.id = reservation.member_id
								 	LEFT JOIN room ON room.id = reservation.assign_room
