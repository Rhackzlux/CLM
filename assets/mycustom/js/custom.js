...
.done(function(data){
    if(data != '') {
        ...
        // Build the select options using <optgroup>
        var selectHtml = "";
        $.each(categories, function(category, items){
            selectHtml += "<optgroup label='" + category + "'>";
            $.each(items, function(i, x){
                selectHtml += "<option value='" + x.id + "'>"
                            + x.brand + " - "
                            + x.stockleft + " QTY" + " [items available] "
                            + "</option>";
            });
            selectHtml += "</optgroup>";
        });

        // Insert into the select box
        $('.borrowitem').html(selectHtml).select2({
