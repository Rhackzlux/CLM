...
$('.add_student').click(function() {
    ...
    var form = '<form class="frm_add_student">\
        ...
        <div class="form-group">\
            ...
            <input type="text" name="s_username" class="form-control" required autocomplete="off">\
        </div>\
        <div class="form-group">\
            <label>Email</label>\
            <input type="email" name="s_email" class="form-control" required autocomplete="off">\
        </div>\
        <div class="form-group">\
 ...
 $('.frm_add_student').submit(function(e) {
    ...
    .done(function(data) {
        if (data.trim() == "1") {
            ...
            toastr.warning('Student already exist.');
        } else if (data.trim() == "Username already exists") {
            toastr.warning('Username already exists.');
        } else if (data.trim() == "School ID Number already exists") {
            toastr.warning('School ID Number already exists.');
        } else if (data.trim() == "Email already exists") {
            toastr.warning('Email already exists.');
        } else if (data.trim() == "Student with this name already exists") {
            toastr.warning('Student with this name already exists.');
        } else {
            toastr.error('Failed to add.');
        }
    });
...
$('.add_faculty').click(function(){
	...
	var mform = '<form class="add_frm_faculty">\
					...
					<div class="form-group">\
						<label>Username</label>\
						<input type="text" name="f_username" class="form-control" required autocomplete="off">\
					</div>\
					<div class="form-group">\
						<label>Password</label>\
